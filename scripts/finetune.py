...
class Trainer:
    ...
    def __load_data(self):
        ...
        self.validation_generator = DataLoader(Dataset(val_labels, self.params, False), **data_loader_parameters)
    ...
    def __validate(self):
        ...
        with torch.no_grad():
            ...
            writer.add_figure("Confusion matrix", ConfusionMatrix(final_preds, final_labels), self.step)
            accuracy = Accuracy(final_preds, final_labels)*100
            f1 = F1_macro(final_preds, final_labels)*100
            loss = loss/len(self.validation_generator)
            writer.add_scalar('Loss/val', loss , self.step)
            writer.add_scalar('Accuracy/val', accuracy  , self.step)
            writer.add_scalar('F1/val', f1, self.step)
            print('--Validation Epoch:{epoch: d}, Updates:{Num_Batch: d}, Loss: {loss: 3.3f}, F1:{f1: 3.3f}, Accuracy:{acc: 3.3f}, elapse:{elapse: 3.3f} min'.format(epoch=self.epoch, Num_Batch=self.step, f1=f1, acc=accuracy, loss=loss, elapse=(time.time()-valid_time)/60))
            ...
            if f1 > self.best_f1:
                ...
                self.stopping += 1
                print('Better F1 is: {}. {} epochs of no improvement'.format(self.best_f1, self.stopping))
            self.print_time = time.time()
            self.net.train()
    ...
    def __update(self):
        ...
        if self.step % int(self.params.print_every) == 0:
            writer.add_scalar('Loss/train', self.train_loss/self.train_batch , self.step)
            writer.add_scalar('Accuracy/train', self.train_accuracy *100/self.train_batch  , self.step)
            writer.add_scalar('F1/train', self.train_f1 *100/self.train_batch, self.step)
            print('Training Epoch:{epoch: d}, Updates:{Num_Batch: d} -----> xent:{xnet: .3f}, Accuracy:{acc: .2f}, F1:{f1: .2f}, elapse:{elapse: 3.3f} min'.format(epoch=self.epoch, Num_Batch=self.step, xnet=self.train_loss / self.train_batch, acc=self.train_accuracy *100/ self.train_batch, f1=self.train_f1 *100/ self.train_batch, elapse=(time.time()-self.print_time)/60))
    ...
    def train(self):
        ...
        for self.epoch in range(self.starting_epoch, self.params.max_epochs):  # loop over the dataset multiple times
            ...
            for input, label in tqdm(self.training_generator):
                ...
                self.train_f1 += F1_macro(prediction, label)
            ...
            if self.stopping > self.params.early_stopping:
                print('--Best Model F1%%: %.2f' %(self.best_f1))
                break
...
if __name__=="__main__":
    ...
    parser.add_argument('--train_labels_path', type = str, default = './labels/limit_speaker/train_files.lst')
    parser.add_argument('--val_labels_path', type = str, default = './labels/limit_speaker/dev_files.lst')
    parser.add_argument('--data_mode', type = str, default = 'normal', choices=['normal','window'])
    parser.add_argument('--out_dir', type=str, default='./models/limit_speaker', help='directory where data is saved')
    ...
    # Network Parameteres
    parser.add_argument('--window_size', type=float, default=1.0, help='number of seconds per window')
